    ...
    for (const auto &q : qpts)
    {
        ...
        // collect the block elements of coulomb matrices
        {
            ...
            for (const auto &IJ : set_IJ_nabf_nabf)
            {
                ...
                //     coul_block, desc_nabf_nabf, LIBRPA::atomic_basis_abf, IJ.first,
                //     IJ.second, true, CONE, IJq_coul.at(I).at({J, qa}).ptr(), MAJOR::ROW);
                collect_block_from_ALL_IJ_Tensor(coul_block, desc_nabf_nabf,
                                                 LIBRPA::atomic_basis_abf, qa, true, CONE, IJq_coul,
                                                 MAJOR::ROW);
                // lib_printf("myid %d I %d J %d nr %d nc %d\n%s",
                //        blacs_ctxt_global_h.myid, I, J,
                //        coul_block.nr(), coul_block.nc(),
        ...
        for (const auto &freq : chi0.tfg.get_freq_nodes())
        {
            ...
            if (epsmac_LF_imagfreq.size() > 0 && is_gamma_point(q))
            {
                ...
                ScalapackConnector::pgemm_f(
                    ...
                    desc_nabf_nabf.desc, sqrtveig_blacs.ptr(), 1, n_singular + 1,
                    desc_nabf_nabf.desc, 0.0, coul_chi0_block.ptr(), 1, 1, desc_nabf_nabf.desc);
                ScalapackConnector::pgemm_f(
                    'C', 'N', n_nonsingular, n_nonsingular, n_abf, 1.0, sqrtveig_blacs.ptr(), 1,
                    n_singular + 1, desc_nabf_nabf.desc, coul_chi0_block.ptr(), 1, 1,
                    desc_nabf_nabf.desc, 0.0, chi0_block.ptr(), 1, 1, desc_nabf_nabf.desc);
                ...
                // rotate back to ABF
                ScalapackConnector::pgemm_f(
                    'N', 'N', n_abf, n_nonsingular, n_nonsingular, 1.0, coul_eigen_block.ptr(), 1,
                    n_singular + 1, desc_nabf_nabf.desc, chi0_block.ptr(), 1, 1,
                    desc_nabf_nabf.desc, 0.0, coul_chi0_block.ptr(), 1, 1, desc_nabf_nabf.desc);
                ScalapackConnector::pgemm_f(
                    'N', 'C', n_abf, n_abf, n_nonsingular, 1.0, coul_chi0_block.ptr(), 1, 1,
                    desc_nabf_nabf.desc, coul_eigen_block.ptr(), 1, n_singular + 1,
                    desc_nabf_nabf.desc, 0.0, chi0_block.ptr(), 1, 1, desc_nabf_nabf.desc);
            }
            else
            {
...
=======
            map_block_to_IJ_storage(Wc_MNmap, LIBRPA::atomic_basis_abf, LIBRPA::atomic_basis_abf,
                                    chi0_block, desc_nabf_nabf, MAJOR::ROW);
>>>>>>> Fix bug: wing is affected by inadequate precision of E_n in 'band_out'. Now agree with FHI-aims, but abacus cannot.
...
map<double, atom_mapping<std::map<Vector3_Order<double>, matrix_m<complex<double>>>>::pair_t_old>
compute_Wc_freq_q_blacs_wing(const Chi0 &chi0, const atpair_k_cplx_mat_t &coulmat_eps,
                             atpair_k_cplx_mat_t &coulmat_wc,
                             const vector<std::complex<double>> &epsmac_LF_imagfreq)
{
    map<double,
    ...
    const TFGrids &tfg, vector<Vector3_Order<int>> Rlist)
{
    ...
    if (mpi_comm_global_h.is_root()) lib_printf("Converting Wc(q,w) -> W(R,t)\n");
    mpi_comm_global_h.barrier();
...
#pragma omp parallel for schedule(dynamic)
    ...
    if (mpi_comm_global_h.is_root()) lib_printf("Done converting Wc q,w -> R,t\n");
    mpi_comm_global_h.barrier();
