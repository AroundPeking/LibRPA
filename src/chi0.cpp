...
#pragma omp parallel for schedule(dynamic)
        for (int I = 0; I != natom; I++)
        {
            ...
            for (int J = 0; J != natom; J++)
            {
                ...
                {
                    // LIBRPA::utils::lib_printf("Discarding GF of spin %1d, IJR {%d, %d, (%d, %d,
                    // %d)}, t %f\n",
                                   ...
                                   const vector<Vector3_Order<double>> &qlist)
{
    ...
    {
        if (mpi_comm_global_h.is_root()) cout << "Use LibRI for chi0" << endl;
                                                 ...
                                                 const vector<Vector3_Order<double>> &qlist)
{
    ...
    auto Rlist_local =
        dispatch_vector(Rlist_gf, mpi_comm_global_h.myid, mpi_comm_global_h.nprocs, true);
    ...
    mpi_comm_global_h.barrier();
    const auto IJRs_gf_local =
        dispatch_vector_prod(get_atom_pair(gf_is_R_tau[0]), Rlist_gf, mpi_comm_global_h.myid,
    ...
    for (auto it = 0; it != tfg.size(); it++)
    {
        ...
        for (const auto &isp : gf_is_R_tau)
        {
            ...
            for (const auto &IJR_gf : IJRs_gf_local)
            {
                ...
                // cout << I << " " << J << " " << Ra << endl;
                if (gf_IJR_tau.count(I) && gf_IJR_tau.at(I).count(J) &&
                    gf_IJR_tau.at(I).at(J).count(R))
                {
                    // positive tau
                    const map<double, matrix> &gf_tau = gf_IJR_tau.at(I).at(J).at(R);
                    if (gf_tau.count(tau) * gf_tau.count(-tau) == 0) continue;
                    // std::valarray<double> mat_po_array(gf_tau.at(tau).c, gf_tau.at(tau).size);
                    ofs_myid << "gf_libri 1,    tau:" << tau << "  I J R: " << I << "  " << J
                             << "  " << R << " size:" << gf_tau.at(tau).size << "\n";
                    std::shared_ptr<std::valarray<double>> mat_po_ptr =
                        std::make_shared<std::valarray<double>>(gf_tau.at(tau).c,
                                                                gf_tau.at(tau).size);
                    // *mat_po_ptr=mat_po_array;
                    gf_po_libri[I][{J, Ra}] = RI::Tensor<double>(
                        {size_t(gf_tau.at(tau).nr), size_t(gf_tau.at(tau).nc)}, mat_po_ptr);
                    // negative tau
                    // std::valarray<double> mat_ne_array(gf_tau.at(-tau).c, gf_tau.at(-tau).size);
                    std::shared_ptr<std::valarray<double>> mat_ne_ptr =
                        std::make_shared<std::valarray<double>>(gf_tau.at(-tau).c,
                                                                gf_tau.at(-tau).size);
                    // *mat_ne_ptr=mat_ne_array;
                    gf_ne_libri[I][{J, Ra}] = RI::Tensor<double>(
                        {size_t(gf_tau.at(-tau).nr), size_t(gf_tau.at(-tau).nc)}, mat_ne_ptr);
                    // LIBRPA::fout_para << "gf_libri 2,    tau:" << tau << "  I J R: "<<I<<"
                    // "<<J<<"  "<<R<<"\n"; count_gf+=1;
                    //             if(count_gf==500)
                    //             {
            ...
            auto chi0s_IJR = RI::Communicate_Tensors_Map_Judge::comm_map2_first(
            ...
            cout << "is: " << isp.first << " tau: " << tau << "  qifreq_atpair_all.size()"
                 << qifreq_atpair_all.size() << endl;
#pragma omp parallel for schedule(dynamic)
        ...
        for (auto atpair : atpairs_ABF)
        {
            ...
            for (int is = 0; is != mf.get_n_spins(); is++)
            {
                ...
                chi0_tau = 2.0 / mf.get_n_spins() *
                           compute_chi0_s_munu_tau_R(LRI_Cs, R_period, is, Mu, Nu, tau, R);
        ...
        for (size_t atom_pair = 0; atom_pair != atpairs_ABF.size(); atom_pair++)
        {
            ...
            for (int is = 0; is != mf.get_n_spins(); is++)
                for (auto &R : Rlist_gf)
                {
                    ...
                    {
                        double tau = tfg.get_time_nodes()[it];
                        ComplexMatrix tmp_chi0_tau(2.0 / mf.get_n_spins() *
                                                   ComplexMatrix(compute_chi0_s_munu_tau_R(
                                                       LRI_Cs, R_period, is, Mu, Nu, tau, R)));
