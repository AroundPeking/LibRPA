...
void set_ao_basis_wfc(int ispin, int ik, double* wfc_real, double* wfc_imag, MeanField& mf)
{
    ...
    for (int i = 0; i != mf.get_n_bands() * mf.get_n_aos(); i++)
    {
                      ...
                      int insert_index_only)
{
    ...
    if (Cs_data.use_libri)
    {
        ...
        // RI tensor uses ABF as slowest index, so we need transpose first with the help of matrix
        // object. This is equivalent to convert the data layout from original row-major to
        // column-major.
