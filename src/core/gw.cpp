...
bool disable_sigc_rspace_symmetry_diagnostic_requested(const char *value)
{
    ...
    throw std::invalid_argument(
        "LIBRPA_DISABLE_SIGC_RSPACE_SYMMETRY_DIAG accepts only the explicit value 'enabled'");
}

bool should_use_sigc_rspace_symmetry(const bool symmetry_available, const bool band_space_complete,
                                     const char *disable_value, const char *qmember_value)
{
    const bool single_q_member = qmember_value != nullptr && qmember_value[0] != '\0';
    return symmetry_available && band_space_complete &&
           !disable_sigc_rspace_symmetry_diagnostic_requested(disable_value) && !single_q_member;
}

bool should_enter_wc_rf_collective(const bool output_wc_rf, const bool local_frequency_available)
{
    (void)local_frequency_available;
    return output_wc_rf;
}

bool direct_compressed_sigc_diagnostic_requested(const char *value)
{
    if (value == nullptr || value[0] == '\0') return false;
    if (std::string(value) == "enabled") return true;
    throw std::invalid_argument(
        "LIBRPA_DIRECT_COMPRESSED_SIGC_DIAG accepts only the explicit value 'enabled'");
}

bool should_contract_sigc_in_compressed_abfs(const bool use_shrink_abfs, const char *value)
{
...
static void add_phase_weighted_sigc_ijk(const Vector3_Order<int> &R_bvk,
                                        const Vector3_Order<double> &kfrac_ik, const double weight,
                                        const Matz &sigc, Matz &sigc_ijk)
{
...
static std::vector<int> collect_target_iks(
    const MpiCommHandler &comm_h,
    const std::map<int, std::map<int, std::map<int, ComplexMatrix>>> &wfc_target, const int n_kpts)
{
    ...
    if (n_kpts > 0) comm_h.allreduce(has_local.data(), has_global.data(), n_kpts, MPI_MAX);
...
static std::string make_sigc_ks_imagfreq_band_stem(const std::string &output_dir, const int index)
{
    ...
    ss << path_as_directory(output_dir) << "self_energy_omega_band_" << std::setfill('0')
       << std::setw(5) << index;
    return ss.str();
}

static std::vector<std::string> make_sigc_rf_filenames(const std::string &input_dir,
                                                       const int ispin, const int ispinor_bra,
                                                       const int ispinor_ket, const int n_spinor,
                                                       const int iomega, const int myid)
{
    const auto dir = path_as_directory(input_dir);
    std::vector<std::string> names;

    std::ostringstream ss;
    ss << dir << "SigcRF_ispin_" << std::setfill('0') << std::setw(2) << ispin << "_s_"
       << std::setw(1) << ispinor_bra << std::setw(1) << ispinor_ket << "_iomega_"
       << std::setfill('0') << std::setw(3) << iomega << "_myid_" << std::setfill('0')
       << std::setw(5) << myid << ".dat";
    ...
    ss << "_iomega_" << std::setfill('0') << std::setw(5) << iomega << "_myid_" << std::setfill('0')
       << std::setw(5) << myid << ".dat";
    names.emplace_back(ss.str());
...
static std::pair<int, int> sigc_rf_file_range_for_rank(const int nfiles, const int nprocs,
                                                       const int myid)
{
    int begin = 0;
...
static std::string find_sigc_rf_file(const std::string &input_dir, const int ispin,
                                     const int ispinor_bra, const int ispinor_ket,
                                     const int n_spinor, const int iomega, const int myid,
                                     bool *found)
{
    const auto candidates =
        make_sigc_rf_filenames(input_dir, ispin, ispinor_bra, ispinor_ket, n_spinor, iomega, myid);
...
convert_symmetry_irreducible_sector_to_libri_gw(
    const symmetry_irreducible_sector_t &irreducible_sector, const std::array<int, 3> &period)
{
    auto canonicalize_r = [&period](const std::array<int, 3> &r)
    {
        ...
        return std::array<int, 3>{centered_mod(r[0], period[0]), centered_mod(r[1], period[1]),
...
public:
    ...
    OutputOnlyFilter_GW_Symmetry(
        const TC &period, const std::map<std::pair<TA, TA>, std::set<TC>> &irreducible_sector)
        : symmetry_(period, irreducible_sector)
    {
    }

    bool filter_for32(const RI::Label::ab_ab &label, const TAC &A1, const TAC &,
    ...
    bool filter_for32(const RI::Label::ab_ab &label, const TAC &A1, const TA &,
                      const TAC &A3) const override
    {
        switch (label)
        {
    ...
    bool filter_for32(const RI::Label::ab_ab &label, const TA &A1, const TAC &,
                      const TAC &A3) const override
    {
        switch (label)
        {
...
static RI::Tensor<Tdata> convert_complex_matrix_to_libri_tensor_gw(const ComplexMatrix &matrix)
{
...
restore_symmetry_ao_rspace_tensor_map_gw(
    const std::map<int, std::map<std::pair<int, std::array<int, 3>>, RI::Tensor<Tdata>>>
        &tensors_ir,
    const SymmetryContext &symmetry_ctx, const symmetry_rspace_sector_stars_t &sector_stars,
    const AtomicBasis &atbasis_wfc)
{
    ...
    for (const auto &i_entry : tensors_ir)
    {
        ...
        for (const auto &jr_entry : i_entry.second)
        {
            ...
            const Vector3_Order<int> ir_R{jr_entry.first.second[0], jr_entry.first.second[1],
                                          jr_entry.first.second[2]};
            ...
            if (pair_iter == sector_stars.end() || pair_iter->second.count(ir_R) == 0)
            {
                ...
                oss << "Failed to match a symmetry-filtered GW self-energy block with the"
                    << " irreducible-sector restore map for I=" << ir_I << " J=" << ir_J << " R=("
                    << ir_R.x << "," << ir_R.y << "," << ir_R.z << ")";
            ...
            for (const auto &restore_member : pair_iter->second.at(ir_R))
            {
                ...
                if (!target.empty())
                {
                    throw std::runtime_error(
                        "Duplicate full-sector GW self-energy block appears during symmetry "
                        "restore");
                }
    ...
    atom_mapping<std::map<Vector3_Order<double>, Matz>>::pair_t_old &Wc_q)
{
    ...
    for (const auto atom_i : atoms_row)
    {
        ...
        for (const auto &atom_j_pair : Wc_q.at(atom_i)) atoms_col.push_back(atom_j_pair.first);
        ...
        for (const auto atom_j : atoms_col)
        {
            for (const auto &q_block : Wc_q.at(atom_i).at(atom_j))
            {
                ...
                else
                {
                    ...
                    hermitian_block = (hermitian_block + hermitian_block.get_transpose(true)) * 0.5;
...
{
    if (start < 0) throw LIBRPA_RUNTIME_ERROR("ifreq_output_wc_start must be non-negative");
    if (start >= n_freq)
    ...
    for (const auto &[I, J_RWc] : Wc_R)
    {
        for (const auto &[J, R_Wc] : J_RWc)
        {
            for (const auto &[R, Wc] : R_Wc)
            {
                ...
                ss << path_as_directory(output_dir) << "Wc_Mu_" << I << "_Nu_" << J << "_iR_" << iR
                   << "_ifreq_" << ifreq << ".mtx";
...
static void write_wc_rf_full_matrix_from_atom_blocks(
    const atom_mapping<std::map<Vector3_Order<int>, Matz>>::pair_t_old &Wc_R,
    const AtomicBasis &atbasis_abf, const ArrayDesc &ad_Wc, const PeriodicBoundaryData &pbc,
    const std::string &output_dir, const int ifreq, const double freq)
{
    for (const auto &R : pbc.Rlist)
    {
        ...
        for (const auto &[I, J_RWc] : Wc_R)
        {
            for (const auto &[J, R_Wc] : J_RWc)
            {
                ...
                collect_block_from_IJ_storage(Wc, ad_Wc, atbasis_abf, atbasis_abf, as_int(I),
                                              as_int(J), cplxdb{1.0, 0.0}, Wc_iter->second.ptr(),
                                              Wc_iter->second.major());
            }
        }
        ...
        ss << path_as_directory(output_dir) << "Wc_iR_" << std::setfill('0') << std::setw(5) << iR
           << "_ifreq_" << std::setfill('0') << std::setw(5) << ifreq << ".mtx";
...
static void write_sigc_matrix_binary(const Matz &mat, const std::string &fn)
{
    ...
    if (!ofs) throw LIBRPA_RUNTIME_ERROR("failed to open SigC matrix output file: " + fn);
...
static void write_sigc_matrix_binary_parallel(const Matz &mat_loc, const ArrayDesc &desc,
                                              const std::string &fn)
{
    ...
    ScalapackConnector::pgemr2d_f(desc.m(), desc.n(), mat_loc.ptr(), 1, 1, desc.desc,
                                  mat_full.ptr(), 1, 1, desc_full.desc, desc.ictxt());
    if (desc_full.is_src()) write_sigc_matrix_binary(mat_full, fn);
    desc.barrier();
}
...
static void write_sigc_nao_kf_matrix(const Matz &mat, const ArrayDesc &desc,
                                     ...
                                     const int n_spinor, const int ik, const int ifreq)
{
    std::ostringstream ss;
    ss << path_as_directory(output_dir) << "SigcKF_" << source << "_ispin_" << ispin;
    if (n_spinor > 1)
    {
        ss << "_spinor_" << ispinor_bra << "_" << ispinor_ket;
    }
...
G0W0::G0W0(const MeanField &mf_in, const AtomicBasis &atbasis_wfc_in,
           ...
           const KPointBlacsParallelContext &band_kblacs_ctxt_in, const ArrayDesc &desc_wfc_in,
           bool is_eigvec_k_distributed, const bool use_symmetry_context_in)
    : mf(mf_in),
...
void G0W0::read_sigc(const std::string &input_dir)
{
    ...
    global::lib_printf_root(
        "Detected SigcRF files from %d MPI rank(s); current job has %d rank(s).\n", nprocs_old,
        global::size_global);
                    ...
                    for (int myid_old = myid_old_begin; myid_old != myid_old_end; ++myid_old)
                    {
                        ...
                        for (size_t idx = 0; idx != n_IJR_myid; ++idx)
                        {
                            ...
                            if (dims[0] >= pbc.Rlist.size())
                                throw LIBRPA_RUNTIME_ERROR(
                                    "SigC checkpoint R index is out of range: " + fn);
                            if (dims[1] >= atbasis_wfc.n_atoms || dims[2] >= atbasis_wfc.n_atoms)
                                throw LIBRPA_RUNTIME_ERROR(
                                    "SigC checkpoint atom index is out of range: " + fn);
                            ...
                            if (dims[3] != n_I || dims[4] != n_J)
                                throw LIBRPA_RUNTIME_ERROR("SigC checkpoint block size mismatch: " +
                                                           fn);
    ...
    comm_h.barrier();
    global::lib_printf_root(
        "Finished reading real-space imaginary-frequency NAO sigma_c matrices.\n");
...
static void build_gf_libri_kpara(
    const MeanField &mf, const MpiCommHandler &comm_h, const AtomicBasis &atbasis_wfc, int ispin,
    int ispinor_bra, int ispinor_ket, const vector<Vector3_Order<double>> &kfrac_list,
    const std::vector<double> &taus,
    const std::vector<std::pair<atpair_t, Vector3_Order<int>>> IJRs,
    std::map<double, std::map<int, std::map<std::pair<int, std::array<int, 3>>, RI::Tensor<Tdata>>>>
        &tau_gf_libri)
{
    ...
    for (const auto &[R, _] : map_R_IJs) Rs_this.emplace_back(R);
    const int n_Rs_this = map_R_IJs.size();
    int n_Rs_max = n_Rs_this;
    comm_h.allreduce(MPI_IN_PLACE, &n_Rs_max, 1, MPI_MAX);
    auto gf_taus_Rs_cplx = get_gf_cplx_imagtimes_Rs_kpara(ispin, ispinor_bra, ispinor_ket, mf,
                                                          kfrac_list, taus, Rs_this, comm_h);
...
#pragma omp parallel for schedule(dynamic)
            for (const auto &IJ : map_IJs)
            {
                ...
                const auto gf_block =
                    get_ap_block_from_global(gf_cplx, IJ, atbasis_wfc, atbasis_wfc);
                if constexpr (is_complex<Tdata>())
                {
                    auto p_gfmat =
                        std::make_shared<std::valarray<Tdata>>(gf_block.c, gf_block.size);
                    omp_set_lock(&gf_lock);
                    tau_gf_libri[tau][I][{J, Ra}] =
                        RI::Tensor<Tdata>({as_size(gf_block.nr), as_size(gf_block.nc)}, p_gfmat);
                    omp_unset_lock(&gf_lock);
                }
                else
                {
                    auto p_gfmat =
                        std::make_shared<std::valarray<Tdata>>(gf_block.real().c, gf_block.size);
                    omp_set_lock(&gf_lock);
                    tau_gf_libri[tau][I][{J, Ra}] =
                        RI::Tensor<Tdata>({as_size(gf_block.nr), as_size(gf_block.nc)}, p_gfmat);
                    omp_unset_lock(&gf_lock);
...
static void build_gf_libri_kserial(
    const MeanField &mf, const AtomicBasis &atbasis_wfc, int ispin, int ispinor_bra,
    int ispinor_ket, const PeriodicBoundaryData &pbc, const SymmetryContext &symmetry_context,
    const bool use_symmetry_context, const vector<Vector3_Order<double>> &kfrac_list,
    const std::vector<double> &taus,
    const std::vector<std::pair<atpair_t, Vector3_Order<int>>> IJRs,
    std::map<double, std::map<int, std::map<std::pair<int, std::array<int, 3>>, RI::Tensor<Tdata>>>>
        &tau_gf_libri)
{
    ...
    bool restore_symmetry_kstars_from_full_grid = !full_grid_kstar_representatives.empty();
    const bool restore_symmetry_kstars =
        can_try_symmetry_kstar_restore && !restore_symmetry_kstars_from_full_grid &&
        can_restore_symmetry_kstar_meanfield(symmetry_context, wfc_layouts, mf, kfrac_list,
                                             atom_nw);
    auto member_kfrac_targets =
        restore_symmetry_kstars_from_full_grid
            ? build_symmetry_full_grid_kstar_member_kfrac_targets(symmetry_context, kfrac_list)
        : restore_symmetry_kstars ? build_symmetry_kstar_member_kfrac_targets(symmetry_context, pbc)
    ...
    if (restore_symmetry_kstars_from_full_grid && !taus.empty() && !Rs_vec.empty())
    {
        ...
        const auto restored_check = get_symmetry_restored_gf_cplx_imagtimes_Rs(
                                        symmetry_context, wfc_layouts, mf, ispin, ispinor_bra,
                                        ispinor_ket, kfrac_list, tau_check, R_check, atom_nw, -1,
                                        &member_kfrac_targets, &full_grid_kstar_representatives)
                                        .at(tau_check.front())
                                        .at(R_check.front());
        const auto direct_check = mf.get_gf_cplx_imagtimes_Rs(ispin, ispinor_bra, ispinor_ket,
                                                              kfrac_list, tau_check, R_check)
                                      .at(tau_check.front())
                                      .at(R_check.front());
    ...
    auto gf = (restore_symmetry_kstars || restore_symmetry_kstars_from_full_grid)
                  ? get_symmetry_restored_gf_cplx_imagtimes_Rs(
                        symmetry_context, wfc_layouts, mf, ispin, ispinor_bra, ispinor_ket,
                        kfrac_list, taus, Rs_vec, atom_nw, -1, &member_kfrac_targets,
                        restore_symmetry_kstars_from_full_grid ? &full_grid_kstar_representatives
                                                               : nullptr)
                  : mf.get_gf_cplx_imagtimes_Rs(ispin, ispinor_bra, ispinor_ket, kfrac_list, taus,
                                                Rs_vec);
    ...
    for (const auto &IJR : IJRs)
    {
        ...
        for (auto t : taus)
        {
            ...
            std::shared_ptr<std::valarray<Tdata>> mat_ptr =
                std::make_shared<std::valarray<Tdata>>(n_I * n_J);
            if constexpr (is_complex<Tdata>())
            {
            ...
            tau_gf_libri[t][as_int(I)][{as_int(J), {R.x, R.y, R.z}}] =
...
static void build_gf_libri_kblacs_para(
    const MeanField &mf, const KPointBlacsParallelContext &kblacs_ctxt, const ArrayDesc &desc_wfc,
    const ArrayDesc &desc_gf, const IndexScheduler &sched, const AtomicBasis &atbasis_wfc,
    int ispin, int ispinor_bra, int ispinor_ket, const PeriodicBoundaryData &pbc,
    const SymmetryContext &symmetry_context, const bool use_symmetry_context,
    const vector<Vector3_Order<double>> &kfrac_list, const std::vector<double> &taus,
        ...
        &tau_gf_libri)
{
    ...
    tau_gf_libri.clear();
    for (auto tau : taus) tau_gf_libri[tau] = {};
    ...
    auto gf_taus_Rs_cplx =
        restore_symmetry_kstars
            ? get_symmetry_restored_gf_cplx_imagtimes_Rs_kblacs_para(
                  ispin, ispinor_bra, ispinor_ket, mf, kfrac_list, taus, Rs, kblacs_ctxt, desc_wfc,
                  desc_gf, symmetry_context, pbc, atbasis_wfc)
            : get_gf_cplx_imagtimes_Rs_kblacs_para(ispin, ispinor_bra, ispinor_ket, mf, kfrac_list,
                                                   taus, Rs, kblacs_ctxt, desc_wfc, desc_gf);
    ...
    for (auto &tau_gf_R_cplx : gf_taus_Rs_cplx)
    {
        ...
        for (auto &R_gf_cplx : gf_R_cplx)
        {
            ...
            auto pair_mat = get_ap_map_from_blacs_dist_scheduler(mat_blacs, sched, atbasis_wfc,
                                                                 atbasis_wfc, desc_gf);
...
void G0W0::build_spacetime(
    ...
    std::map<double, atom_mapping<std::map<Vector3_Order<double>, Matz>>::pair_t_old>
        *Wc_freq_q_atom_pair,
    std::map<Vector3_Order<double>, ComplexMatrix> *sinvS, const AtomicBasis *basis_aux_compressed,
    const AtomicBasis *basis_aux_unfold, const BlacsCtxtHandler *blacs_ctxt_h,
    const ArrayDesc *desc_wfc_in)
{
    ...
    const bool use_complex_tensor = mf.get_n_spinor() > 1 || force_complex_spacetime;
    if (force_complex_spacetime)
    {
        lib_printf_root(
            "Diagnostic: complex GW space-time tensors are forced; physical inputs and "
            "symmetry remain unchanged\n");
    }
    ...
    if (comm_h.myid == 0)
    {
        std::cout << "LIBRA::G0W0::build_spacetime is only implemented on top of LibRI"
                  << std::endl;
        std::cout << "Please recompiler LibRPA with -DUSE_LIBRI and configure include path"
                  << std::endl;
    }
    comm_h.barrier();
    ...
    const bool direct_compressed_sigc = should_contract_sigc_in_compressed_abfs(
        use_atom_pair_Wc, std::getenv("LIBRPA_DIRECT_COMPRESSED_SIGC_DIAG"));

    typedef std::map<int, std::map<std::pair<int, std::array<int, 3>>, RI::Tensor<double>>>
        dtensor_map;
    typedef std::map<int, std::map<std::pair<int, std::array<int, 3>>, RI::Tensor<cplxdb>>>
        ztensor_map;
    ...
    if (use_atom_pair_Wc)
    {
        if (basis_aux_compressed == nullptr || desc_wfc_in == nullptr ||
            (!direct_compressed_sigc &&
             (sinvS == nullptr || basis_aux_unfold == nullptr || blacs_ctxt_h == nullptr)))
            throw LIBRPA_RUNTIME_ERROR(
                "shrink Wc build_spacetime requires sinvS, compressed/full ABF bases, BLACS "
                "context, and WFC descriptor");

        std::unique_ptr<Chi0> unfold_helper;
        if (!direct_compressed_sigc)
            unfold_helper = std::make_unique<Chi0>(
                mf, atbasis_wfc, *basis_aux_compressed, pbc, symmetry_context, tfg, kblacs_ctxt,
                *desc_wfc_in, is_eigvec_k_distributed_, this->use_symmetry_context);

        if (output_wc_rf)
        {
            const int ifreq_end = wc_rf_checked_ifreq_end(ifreq_output_wc_start,
                                                          ifreq_output_wc_end, tfg.get_n_grids());
            ...
            for (int ifreq = ifreq_output_wc_start; ifreq != ifreq_end; ++ifreq)
            {
                ...
                const bool local_frequency_available = freq_iter != Wc_freq_q_atom_pair->end();
                if (!should_enter_wc_rf_collective(output_wc_rf, local_frequency_available))
                    continue;

                // Wc is distributed by atom pair. Ranks without a local block at
                // this frequency must still enter the unfold and output
                // collectives, otherwise their peers deadlock in comm_map2_first.
                atom_mapping<std::map<Vector3_Order<double>, Matz>>::pair_t_old Wc_q;
                if (local_frequency_available) Wc_q = freq_iter->second;
                ...
                if (output_wc_rf_atom_pair)
                    ...
                    write_wc_rf_full_matrix_from_atom_blocks(Wc_R, atbasis_abf, ad_Wc, pbc,
                                                             output_dir, ifreq, freq);
            }
            profiler.stop("write_Wc_freq_R");
        }

        profiler.start("g0w0_build_spacetime_wt_ft_wc", "Tranform Wc (q,w) -> (q,t)");
        auto Wc_tau_q = CT_Wc_freq2time_q(comm_h, *basis_aux_compressed, *Wc_freq_q_atom_pair, tfg,
                                          pbc.get_n_cells_bvk(), pbc.Rlist, pbc.klist_coul);
        ...
        for (auto itau = 0; itau != n_tau; ++itau)
        {
            ...
            const auto &contraction_basis =
                direct_compressed_sigc ? atbasis_abf : *basis_aux_unfold;
            auto Wc_R = FT_Wc_q2R(comm_h, contraction_basis, symmetry_context, Wc_q, tfg, pbc,
                                  pbc.Rlist, false, output_dir, this->use_symmetry_context);
            ...
            for (auto &[I, J_RWc] : Wc_R)
            {
                ...
                for (auto &[J, R_Wc] : J_RWc)
                {
                    ...
                    for (auto &[R, Wc_block] : R_Wc)
                    {
                        ...
                        if (use_complex_tensor)
                        {
                            const auto Wc_IJ_minus_R = minus_R_iter->second.get_transpose(true);
    ...
    else  // !use_atom_pair_Wc
    {
        ...
        auto Wc_tau_R_blacs =
            CT_FT_Wc_freq_q(comm_h, Wc_freq_q, pbc, tfg, true, output_wc_rf, ifreq_output_wc_start,
                            ifreq_output_wc_end, output_wc_rf_atom_pair, output_dir, &ad_Wc,
                            &atbasis_abf, &qpoint_view, &symmetry_context);
        release_free_mem();
        ...
        ofs_myid << "wc_major == MAJOR::ROW ? " << std::boolalpha << (wc_major == MAJOR::ROW)
                 << std::endl;
        ...
        for (auto &[tau, map_R_mat] : Wc_tau_R_blacs)
        {
            ...
            {
                auto pair_mat = get_ap_map_from_blacs_dist_scheduler(mat_blacs, sched, atbasis_abf,
                                                                     atbasis_abf, ad_Wc);
                for (auto &[pair, mat_ap] : pair_mat)
                {
                    const auto &I = as_int(pair.first);
                    ...
                    if (use_complex_tensor)
                        tau_Wc_libri_cplx[tau][I][{J, {R.x, R.y, R.z}}] =
                            RI::Tensor<cplxdb>({nabf_I, nabf_J}, mat_ap.sptr());
                    else
                        tau_Wc_libri[tau][I][{J, {R.x, R.y, R.z}}] =
                            RI::Tensor<double>({nabf_I, nabf_J}, mat_ap.get_real().sptr());
                }
                mat_blacs.clear();
            }
            profiler.stop("g0w0_build_spacetime_prep_Wc_all");
    ...
    const bool sigc_rspace_symmetry_available =
        this->use_symmetry_context && symmetry_ctx.available &&
        symmetry_species_layouts_match_atom_counts(wfc_layouts, symmetry_ctx.atom_to_type,
                                                   atbasis_wfc.get_atom_nb_map()) &&
        !symmetry_ctx.irreducible_sector.empty() && !symmetry_ctx.rspace_sector_stars.empty() &&
    ...
    const bool sigc_band_space_complete = rspace_symmetry_has_complete_band_space(mf, -1);
    const char *disable_sigc_symmetry_env = std::getenv("LIBRPA_DISABLE_SIGC_RSPACE_SYMMETRY_DIAG");
    const char *qmember_diagnostic_env = std::getenv("LIBRPA_STRICT2D_QMEMBER_DIAG");
    const bool disable_sigc_rspace_symmetry_diagnostic =
        disable_sigc_rspace_symmetry_diagnostic_requested(disable_sigc_symmetry_env);
    const bool qmember_diagnostic_enabled =
        qmember_diagnostic_env != nullptr && qmember_diagnostic_env[0] != '\0';
    const bool use_input_sigc_symmetry =
        should_use_sigc_rspace_symmetry(sigc_rspace_symmetry_available, sigc_band_space_complete,
                                        disable_sigc_symmetry_env, qmember_diagnostic_env);
    if (disable_sigc_rspace_symmetry_diagnostic && comm_h.is_root())
    {
        global::lib_printf(
            "Diagnostic: GW real-space self-energy irreducible-sector output filtering and "
            "restoration are disabled; k-star and q-star symmetry remain active\n");
    }
    if (qmember_diagnostic_enabled && comm_h.is_root())
    {
        global::lib_printf(
            "Diagnostic: GW real-space self-energy symmetry filtering and restoration are "
            "disabled because a single full-BZ q member does not preserve crystal symmetry\n");
    }
    ...
    const auto libri_sigc_irreducible_sector =
        use_input_sigc_symmetry ? convert_symmetry_irreducible_sector_to_libri_gw(
    ...
    auto IJR_local_gf =
        dispatch_vector_prod(tot_atpair_ordered, Rlist, ad_Wc.myid(), ad_Wc.nprocs(), true, false);
    ...
    if (this->is_eigvec_k_distributed_)
    {
        ...
        for (const auto &iR : iRs) Rs_gf.push_back(Rlist[iR]);
        ...
        for (int ispin = 0; ispin != mf.get_n_spins(); ispin++)
        {
            for (int ispinor_bra = 0; ispinor_bra < n_spinor; ispinor_bra++)
            {
                for (int ispinor_ket = 0; ispinor_ket < n_spinor; ispinor_ket++)
                {
                    ...
                    // global::ofs_myid << "t " << gf[tau].size() << " ; -t " << gf[-tau].size() <<
                    // endl;
                    ...
                    if (use_complex_tensor)
                    {
                        if (this->is_eigvec_k_distributed_)
                        {
                            build_gf_libri_kblacs_para(
                                mf, kblacs_ctxt, desc_wfc, desc_gf, sched_gf, atbasis_wfc, ispin,
                                ispinor_bra, ispinor_ket, this->pbc, this->symmetry_context,
                                this->use_symmetry_context, this->pbc.kfrac_list, taus, Rs_gf,
                                tau_gf_libri_cplx);
                        }
                        else
                        {
                            build_gf_libri_kserial(mf, atbasis_wfc, ispin, ispinor_bra, ispinor_ket,
                                                   this->pbc, this->symmetry_context,
                                                   this->use_symmetry_context, this->pbc.kfrac_list,
                    ...
                    {
                        lib_printf_root(
                            "Time for Green's function, i_spin %d i_spinor_bra %d i_spinor_ket %d "
                            "i_tau %d (seconds, Wall/CPU): %f %f\n",
                    ...
                    for (auto t : taus)
                    {
                        ...
                        if (use_complex_tensor)
                        {
                            ...
                            {
                                gw_libri_cplx.Sigmas = restore_symmetry_ao_rspace_tensor_map_gw(
                                    gw_libri_cplx.Sigmas, symmetry_ctx, symmetry_sector_stars,
                                    this->atbasis_wfc);
                            }
                            release_free_mem();
                            ...
                            global::ofs_myid << "Temporary Sigc_tau size for time " << t
                                             << " [MB]: " << mem_mb << std::endl;
                        ...
                        else
                        {
                            ...
                            // if (t > 0)  // debug
                            // {
                            //     global::ofs_myid << "gf_libri posi ispin=0 itau " << itau << " "
                            //     << get_num_keys(gf_libri)  << std::endl;
                            ...
                            //         {
                            //             std::stringstream ss;
                            //             Vector3_Order<int> R(JR.second[0], JR.second[1],
                            //             JR.second[2]); ss << "gf_libri_posi"
                            //                 << "_itau_" << std::setfill('0') << std::setw(5) <<
                            //                 itau
                            //                 << "_I_" << std::setfill('0') << std::setw(5) << I
                            //                 << "_J_" << std::setfill('0') << std::setw(5) <<
                            //                 JR.first
                            //                 << "_iR_" << std::setfill('0') << std::setw(5) <<
                            //                 get_R_index(pbc.Rlist, R) << ".dat";
                            //             global::ofs_myid << "Writing GF to " << ss.str() << " "
                            //             << tau << " " << I << " " << JR.second << " " << R <<
                            //             std::endl; std::ofstream ofs(ss.str()); ofs << gf <<
                            //             std::endl; ofs.close();
                            //         }
                            //     }
                            // }
                            // else
                            // {
                            //     global::ofs_myid << "gf_libri nega ispin=0 itau " << itau << " "
                            //     << get_num_keys(gf_libri) << std::endl;
                            //     print_keys(global::ofs_myid, gf_libri);
                            //     // global::ofs_myid << gf_libri << std::endl;
                            //     for (const auto &[I, JR_gf]: gf_libri)
                            ...
                            if (restore_input_sigc_output)
                            {
                                gw_libri.Sigmas = restore_symmetry_ao_rspace_tensor_map_gw(
                                    gw_libri.Sigmas, symmetry_ctx, symmetry_sector_stars,
                                    this->atbasis_wfc);
                            }
                            global::profiler.stop("g0w0_build_spacetime_5");
                            global::profiler.start("g0w0_build_spacetime_5_clean",
                            ...
                            //     if (t > 0)
                            //     {
                            //         global::ofs_myid << "sigc_posi_tau itau=0 ispin=0 " <<
                            //         get_num_keys(sigc_posi_tau) << std::endl;
                            //         print_keys(global::ofs_myid, sigc_posi_tau);
                            //         global::ofs_myid << sigc_posi_tau << std::endl;
                            //     }
                            //     if (t < 0)
                            //     {
                            //         global::ofs_myid << "sigc_nega_tau itau=0 ispin=0 " <<
                            //         get_num_keys(sigc_nega_tau) << std::endl;
                        ...
                        if (n_spinor > 1)
                            global::lib_printf(
                                "Task %4d. libRI G0W0, spin %1d, bra %1d, ket %1d, time grid "
                                "%12.6f. Wc size %zu, GF "
                                "size %zu. Wall time %f\n",
                                comm_h.myid, ispin, ispinor_bra, ispinor_ket, t, n_obj_wc_libri,
                                n_obj_gf_libri, wtime_g0w0_cal_sigc);
                        else
                            global::lib_printf(
                    ...
                    if (this->output_sigc_mat_rt)
                    {
                        ...
                        ss << "_itau_" << std::setfill('0') << std::setw(5) << itau << "_myid_"
                           << std::setfill('0') << std::setw(5) << global::myid_global << ".dat";
                        ofs_sigmac_r.open(ss.str(), std::ios::out | std::ios::binary);
                        ofs_sigmac_r.write((char *)&n_IJR_myid, sizeof(size_t));  // placeholder
                    }

                    auto accumulate_sigc_tau_to_freq =
                        [&](const auto &sigc_posi_tau_in, const auto &sigc_nega_tau_in,
                            auto block_scale_factor, auto make_freq_value, size_t elem_size)
                    {
                        for (const auto &[I, J_R_sigc_posi] : sigc_posi_tau_in)
                        {
                            ...
                            for (const auto &[JR, sigc_posi_block] : J_R_sigc_posi)
                            {
                                ...
                                const auto sigc_cos =
                                    block_scale_factor * (sigc_posi_block + sigc_nega_block);
                                const auto sigc_sin =
                                    block_scale_factor * (sigc_posi_block - sigc_nega_block);
                    ...
                    global::profiler.start("g0w0_build_spacetime_6",
                                           "Transform Sigc (R,t) -> (R,w)");
                    if (use_complex_tensor)
                    {
                        accumulate_sigc_tau_to_freq(
                            sigc_posi_tau_cplx, sigc_nega_tau_cplx, cplxdb(0.5, 0.0),
                    ...
                    {
                        accumulate_sigc_tau_to_freq(
                            sigc_posi_tau, sigc_nega_tau, 0.5,
...
void G0W0::build_sigc_matrix_KS_blacs(
    const std::map<int, std::map<int, std::map<int, ComplexMatrix>>> &wfc_target,
    const std::vector<Vector3_Order<double>> &kfrac_target,
    const AtomPairBvKRemap<atom_t> &bvk_remap, const BlacsCtxtHandler &blacs_ctxt_h,
    const bool use_gpu_replace_scalapack, const std::string &source)
{
    ...
    if (use_klocal_rotation)
    {
        ...
        if (rotation_kblacs_ctxt->n_kpoints() != n_target_kpoints)
            throw LIBRPA_RUNTIME_ERROR(
                "k-point BLACS context has inconsistent number of target k-points");
    }
    ...
    {
        std::cout << "LIBRA::G0W0::build_sigc_matrix_KS is only implemented on top of LibRI"
                  << std::endl;
        std::cout << "Please recompile LibRPA with -DLIBRPA_USE_LIBRI and optionally configure "
                     "include path"
                  << std::endl;
    }
...
#if defined(LIBRPA_USE_CUDA) || defined(LIBRPA_USE_HIP)
    std::complex<double> *d_wfc_bra = nullptr, *d_wfc_ket = nullptr, *d_sigc_nao = nullptr,
                         *d_temp = nullptr, *d_sigc_nband = nullptr;
    ...
    if (use_gpu_replace_scalapack)
    {
        ...
        if (rotation_blacs_h_nc.ddla_handle == nullptr) rotation_blacs_h_nc.init_ddla_handle();
        ...
        size_sigc_nao = static_cast<size_t>(desc_nao_nao_opt.m_loc()) * desc_nao_nao_opt.n_loc();
        size_temp = static_cast<size_t>(desc_nband_nao_opt.m_loc()) * desc_nband_nao_opt.n_loc();
        size_sigc_nband =
            static_cast<size_t>(desc_nband_nband_opt.m_loc()) * desc_nband_nband_opt.n_loc();
        DEVICE_CHECK(deviceMallocAsync((void **)&d_wfc_bra, size_wfc * sizeof(std::complex<double>),
                                       rotation_blacs_h.ddla_handle->stream));
        DEVICE_CHECK(deviceMallocAsync((void **)&d_wfc_ket, size_wfc * sizeof(std::complex<double>),
                                       rotation_blacs_h.ddla_handle->stream));
        DEVICE_CHECK(deviceMallocAsync((void **)&d_sigc_nao,
                                       size_sigc_nao * sizeof(std::complex<double>),
                                       rotation_blacs_h.ddla_handle->stream));
        DEVICE_CHECK(deviceMallocAsync((void **)&d_temp, size_temp * sizeof(std::complex<double>),
                                       rotation_blacs_h.ddla_handle->stream));
        DEVICE_CHECK(deviceMallocAsync((void **)&d_sigc_nband,
    ...
    const bool use_root_dense_projection =
        ...
        use_symmetry_ibz_root_projection(this->symmetry_context, this->pbc, n_target_kpoints,
                                         this->mf.get_n_kpoints());
    if (use_root_dense_projection)
    {
        desc_nao_nao_fb.init(n_aos, n_aos, n_aos, n_aos, 0, 0);
    }
    const auto set_IJ_nao_nao_rotation =
        get_necessary_IJ_from_block_2D(this->atbasis_wfc, this->atbasis_wfc, desc_nao_nao);
                ...
                for (int ispn_ket = 0; ispn_ket < n_spinor; ispn_ket++)
                {
                    ...
                    for (const auto &freq : this->tfg.get_freq_nodes())
                    {
                                ...
                                for (const auto &IJ_R_sigc : it_sp_f->second)
                                {
                                    ...
                                    for (const auto &[R, sigc] : IJ_R_sigc.second)
                                    {
                                        ...
                                        sigc_I_JR_local[I][{J, Ra}] =
                                            Tensor<complex<double>>({n_I, n_J}, sigc.sptr());
                                    }
                                }
                            }
                        }
                        // for certain spin and frequency
                        auto sigc_I_JR = comm_map2_first(comm_h.comm, sigc_I_JR_local, s0_s1.first,
                                                         s0_s1.second);
                        ...
                        if (sigc_dest == nullptr)
                            sigc_is_f_IJ_R_redist[isp][ispn_bra][ispn_ket][freq] =
                                std::move(sigc_new);
                        else
                        {
    ...
    using SigcIJKKey =
        std::pair<SigcIJKAtomKey, int>;  // (J, ik); J stays first for atom-pair routing.
    ...
    auto collect_sigc_nao_from_ijk =
        [this, &desc_nao_nao](Matz &sigc_nao_nao, const std::map<int, std::map<int, Matz>> &sigc_ij)
    {
        ...
        for (const auto &[I, J_sigc] : sigc_ij)
        {
            ...
            {
                collect_block_from_IJ_storage(sigc_nao_nao, desc_nao_nao, this->atbasis_wfc,
                                              this->atbasis_wfc, I, J, complex<double>{1.0, 0.0},
                                              sigc.ptr(), sigc.major());
            }
        }
    };
        ...
        for (int ispn_bra = 0; ispn_bra < n_spinor; ispn_bra++)
        {
            for (int ispn_ket = 0; ispn_ket < n_spinor; ispn_ket++)
            {
                if (use_klocal_rotation)
                {
                    ...
                    auto temp_nband_nao_opt =
                        init_local_mat<complex<double>>(desc_nband_nao_opt, MAJOR::COL);
...
#pragma omp parallel for schedule(dynamic)
                                ...
                                {
                                    const auto &task =
                                        fourier_tasks[static_cast<std::size_t>(itask)];
                                    ...
                                    for (const auto &[R, sigc] : *task.R_sigc)
                                    {
                                        ...
                                        {
                                            add_phase_weighted_sigc_ijk(R, kfrac_ik, 1.0, sigc,
                                                                        sigc_ijk);
                                        }
                                        else if (R_bvks->size() == 1)
                                        {
                                            add_phase_weighted_sigc_ijk(R_bvks->front(), kfrac_ik,
                                                                        1.0, sigc, sigc_ijk);
                                        }
                                        else
                                        {
                                            const auto weight =
                                                1.0 / static_cast<double>(R_bvks->size());
                                            for (const auto &R_bvk : *R_bvks)
                                                add_phase_weighted_sigc_ijk(R_bvk, kfrac_ik, weight,
                                                                            sigc, sigc_ijk);
                                        }
                                    }
                                    fourier_results[static_cast<std::size_t>(itask)] =
                                        std::move(sigc_ijk);
                                }
                        ...
                        std::map<SigcIJKAtomKey, std::map<SigcIJKKey, Tensor<cplxdb>>>
                            sigc_I_Jik_tensor;
                        ...
                        for (const int ik : rotation_kblacs_ctxt->kpoints_local())
                        {
                            if (ik < 0 || ik >= n_target_kpoints)
                                throw LIBRPA_RUNTIME_ERROR(
                                    "k-point index out of range for k-local SigC rotation");
                            ...
                            if (desc_nao_nband_fb.is_src())
                            {
                                ...
                                ScalapackConnector::pgemr2d_f(
                                    n_aos, n_bands, wfc_bra.c, 1, 1, desc_nao_nband_fb.desc,
                                    wfc_bra_opt.ptr(), 1, 1, desc_nao_nband_opt.desc,
                                    desc_nao_nband_fb.ictxt());
                                ScalapackConnector::pgemr2d_f(
                                    n_aos, n_bands, wfc_ket.c, 1, 1, desc_nao_nband_fb.desc,
                            ...
                            {
                                ScalapackConnector::pgemr2d_f(
                                    n_aos, n_bands, dummy.data(), 1, 1, desc_nao_nband_fb.desc,
                                    wfc_bra_opt.ptr(), 1, 1, desc_nao_nband_opt.desc,
                                    desc_nao_nband_fb.ictxt());
                                ScalapackConnector::pgemr2d_f(
                                    n_aos, n_bands, dummy.data(), 1, 1, desc_nao_nband_fb.desc,
                            ...
                            {
                                DEVICE_CHECK(
                                    deviceMemcpyAsync(d_wfc_bra, wfc_bra_opt.ptr(),
                                                      size_wfc * sizeof(std::complex<double>),
                                                      deviceMemcpyHostToDevice,
                                                      rotation_blacs_h.ddla_handle->stream));
                                DEVICE_CHECK(
                                    deviceMemcpyAsync(d_wfc_ket, wfc_ket_opt.ptr(),
                                                      size_wfc * sizeof(std::complex<double>),
                                                      deviceMemcpyHostToDevice,
                                                      rotation_blacs_h.ddla_handle->stream));
                                DEVICE_CHECK(
                                    deviceMemcpyAsync(d_sigc_nao, sigc_nao_nao_opt.ptr(),
                                                      size_sigc_nao * sizeof(std::complex<double>),
                                                      deviceMemcpyHostToDevice,
                                                      rotation_blacs_h.ddla_handle->stream));
                                LaConnector::pgemm('C', 'N', n_bands, n_aos, n_aos,
                                                   std::complex<double>{1.0, 0.0}, d_wfc_bra, 1, 1,
                                                   desc_nao_nband_opt, d_sigc_nao, 1, 1,
                                                   desc_nao_nao_opt, std::complex<double>{0.0, 0.0},
                                                   d_temp, 1, 1, desc_nband_nao_opt);
                                LaConnector::pgemm('N', 'N', n_bands, n_bands, n_aos,
                                                   std::complex<double>{1.0, 0.0}, d_temp, 1, 1,
                                                   desc_nband_nao_opt, d_wfc_ket, 1, 1,
                                                   desc_nao_nband_opt,
                                                   std::complex<double>{0.0, 0.0}, d_sigc_nband, 1,
                                                   1, desc_nband_nband_opt);
                                DEVICE_CHECK(deviceMemcpyAsync(
                                    sigc_nband_nband_opt.ptr(), d_sigc_nband,
                                    size_sigc_nband * sizeof(std::complex<double>),
                                    deviceMemcpyDeviceToHost,
                                    rotation_blacs_h.ddla_handle->stream));
                                DEVICE_CHECK(
                                    deviceStreamSynchronize(rotation_blacs_h.ddla_handle->stream));
                            }
                            else
#endif
                            {
                                ScalapackConnector::pgemm_f(
                                    'C', 'N', n_bands, n_aos, n_aos, 1.0, wfc_bra_opt.ptr(), 1, 1,
                                    desc_nao_nband_opt.desc, sigc_nao_nao_opt.ptr(), 1, 1,
                                    desc_nao_nao_opt.desc, 0.0, temp_nband_nao_opt.ptr(), 1, 1,
                                    desc_nband_nao_opt.desc);
                                ScalapackConnector::pgemm_f(
                                    'N', 'N', n_bands, n_bands, n_aos, 1.0,
                                    temp_nband_nao_opt.ptr(), 1, 1, desc_nband_nao_opt.desc,
                                    wfc_ket_opt.ptr(), 1, 1, desc_nao_nband_opt.desc, 0.0,
                            ...
                            for (int ib = 0; ib != n_bands; ++ib)
                            {
                                ...
                                if (jlo < 0) continue;
                                diag_send[ib] = sigc_nband_nband_opt
                                                    .ptr()[ilo + desc_nband_nband_opt.lld() * jlo];
                            }
                            std::vector<cplxdb> diag_recv(n_bands);
                            rotation_kblacs_ctxt->comm_blacs_h.reduce(
                                diag_send.data(), diag_recv.data(), n_bands, 0, MPI_SUM);
                ...
                else
                {
                    ...
                    for (const auto &freq : this->tfg.get_freq_nodes())
                    {
                        ...
                        // to speed up later Fourier transform while keeping band interpolation
                        // accurate. A missing remap entry means that R is already the desired
                        // counterpart. NOTE: from local redistribution, sigc_rotation_source has
                        // all [spin][freq] keys, but each value (atom-pair map) can be empty.
                        for (const auto &[IJ, R_sigc] : sigc_IJ_R)
                        {
                            const auto &I = IJ.first;
                            const auto &J = IJ.second;
                    ...
                    for (const auto &freq : this->tfg.get_freq_nodes())
                    {
                        ...
                        for (size_t ik = 0; ik < kfrac_target.size(); ik++)
                        {
                            ...
                            // const std::function<complex<double>(const atom_t, const atom_t, const
                            // Vector3_Order<int> &)>
                            const std::function<complex<double>(const atom_t, const atom_t,
                                                                const Vector3_Order<int> &)>
                                fourier = [kfrac](const atom_t I, const atom_t J,
                            ...
                            if (use_root_dense_projection)
                            {
                                ...
                                if (comm_h.is_root())
                                {
                                    ...
                                    const auto &wfc_bra = wfc_target.at(isp).at(ispn_bra).at(ik);
                                    const auto &wfc_ket = wfc_target.at(isp).at(ispn_ket).at(ik);
                                    sigc_nband_nband_dense = conj(wfc_bra) * sigc_nao_nao_dense *
                                                             transpose(wfc_ket, false);
                                }
                                broadcast_ComplexMatrix(sigc_nband_nband_dense, 0, comm_h.comm);
                                if (this->output_sigc_ks_mat_kf)
                                {
                                    const Matz sigc_dense(n_bands, n_bands,
                                                          sigc_nband_nband_dense.c, MAJOR::ROW);
                            ...
                            const auto wfc_block =
                                get_local_mat(wfc_bra.c, MAJOR::ROW, desc_nband_nao, MAJOR::COL)
                                    .conj();
                            auto temp_nband_nao =
                                multiply_scalapack(wfc_block, desc_nband_nao, sigc_nao_nao,
                                                   desc_nao_nao, desc_nband_nao);
                            if (ispn_bra == ispn_ket)
                                ScalapackConnector::pgemm_f(
                                    'N', 'C', n_bands, n_bands, n_aos, 1.0, temp_nband_nao.ptr(), 1,
                                    1, desc_nband_nao.desc, wfc_block.ptr(), 1, 1,
                                    desc_nband_nao.desc, 0.0, sigc_nband_nband.ptr(), 1, 1,
                                    desc_nband_nband.desc);
                            else
                            {
                                const auto wfc_ket_block = get_local_mat(
                                    wfc_ket.c, MAJOR::ROW, desc_nband_nao, MAJOR::COL);
                                ScalapackConnector::pgemm_f(
                                    'N', 'T', n_bands, n_bands, n_aos, 1.0, temp_nband_nao.ptr(), 1,
                                    1, desc_nband_nao.desc, wfc_ket_block.ptr(), 1, 1,
                                    desc_nband_nao.desc, 0.0, sigc_nband_nband.ptr(), 1, 1,
                                    desc_nband_nband.desc);
                            }
                            if (this->output_sigc_ks_mat_kf)
                            {
                                ScalapackConnector::pgemr2d_f(
                                    n_bands, n_bands, sigc_nband_nband.ptr(), 1, 1,
                                    desc_nband_nband.desc, sigc_nband_nband_opt.ptr(), 1, 1,
                                    desc_nband_nband_opt.desc, desc_nband_nband_opt.ictxt());
                                store_sigc_local(isp, ik, freq, sigc_nband_nband_opt);
                            }
                            ...
                            ScalapackConnector::pgemr2d_f(
                                n_bands, n_bands, sigc_nband_nband.ptr(), 1, 1,
                                desc_nband_nband.desc, sigc_nband_nband_fb.ptr(), 1, 1,
                                desc_nband_nband_fb.desc, desc_nband_nband_fb.ictxt());
                            // Only the matrix at the master process is meaningful; extract its
                            // diagonal instead of storing the full n_bands x n_bands matrix.
                            if (comm_h.is_root())
                            {
                                std::vector<cplxdb> diag(n_bands);
                                for (int ib = 0; ib != n_bands; ++ib)
...
{
    comm_h.barrier();
    global::ofs_myid << "build_sigc_matrix_KS_kgrid: constructing self-energy matrix for SCF k-grid"
                     << std::endl;
    if (is_eigvec_k_distributed_)
        throw LIBRPA_RUNTIME_ERROR(
            "G0W0::build_sigc_matrix_KS_kgrid cannot consume k-distributed eigenvectors; use "
            "build_sigc_matrix_KS_kgrid_blacs");
...
void G0W0::build_sigc_matrix_KS_band(
    ...
    const std::vector<Vector3_Order<double>> &kfrac_band, const AtomPairBvKRemap<atom_t> &bvk_remap,
    const std::vector<int> *output_iks)
{
    ...
    if (is_eigvec_k_distributed_)
        throw LIBRPA_RUNTIME_ERROR(
            "G0W0::build_sigc_matrix_KS_band cannot consume k-distributed eigenvectors; use "
            "build_sigc_matrix_KS_band_blacs");
    ...
    if (this->output_sigc_ks_kf)
    {
        ...
        const auto iks = output_iks == nullptr
                             ...
                             : *output_iks;
        const auto stem =
            make_sigc_ks_imagfreq_band_stem(this->output_dir, output_sigc_ks_kf_band_index_++);
                                            ...
                                            const bool use_gpu_replace_scalapack)
{
    comm_h.barrier();
    global::ofs_myid
        << "build_sigc_matrix_KS_kgrid: constructing self-energy matrix for SCF k-grid with BLACS"
        << std::endl;
    this->build_sigc_matrix_KS_blacs(this->mf.get_eigenvectors(), this->pbc.kfrac_list, {},
                                     blacs_ctxt_h, use_gpu_replace_scalapack, "kgrid");
    if (this->output_sigc_ks_kf)
    {
        const auto fn = path_as_directory(this->output_dir) + "self_energy_omega.dat";
        write_self_energy_omega(fn.c_str(), *this, this->mf.get_n_kpoints(),
...
void G0W0::build_sigc_matrix_KS_band_blacs(
    const std::map<int, std::map<int, std::map<int, ComplexMatrix>>> &wfc,
    const std::vector<Vector3_Order<double>> &kfrac_band, const AtomPairBvKRemap<atom_t> &bvk_remap,
    const BlacsCtxtHandler &blacs_ctxt_h, const bool use_gpu_replace_scalapack,
    const std::vector<int> *output_iks)
{
    comm_h.barrier();
    if (comm_h.myid == 0)
    {
        global::lib_printf(
            "build_sigc_matrix_KS_band: constructing self-energy matrix for band k-path with "
            "BLACS\n");
    }
    ...
    if (this->output_sigc_ks_kf)
    {
        ...
        const auto iks = output_iks == nullptr
                             ? collect_target_iks(comm_h, wfc, static_cast<int>(kfrac_band.size()))
                             : *output_iks;
        const auto stem = make_sigc_ks_imagfreq_band_stem(this->output_dir, output_band_index);
        write_self_energy_omega((stem + ".dat").c_str(), *this, iks, n_bands);
        write_self_energy_omega_kpoints((stem + ".kidx").c_str(), *this, iks);
    }
    if (this->output_sigc_ks_kf || this->output_sigc_mat_kf) ++output_sigc_ks_kf_band_index_;
}
