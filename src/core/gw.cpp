                    ...
                    if (geometry.is_frac_set())
                    {
                        ...
                        for (const auto &[IJ, R_sigc]: sigc_IJ_R)
                        {
                            ...
                            for (auto &[R, sigc]: R_sigc)
                            {
                                ...
                                auto &sigc_R = sigc_isp_local[freq][I][J];
                                auto it_sigc_R = sigc_R.find(R_bvk);
                                if (it_sigc_R == sigc_R.end())
                                {
                                    sigc_R.emplace(R_bvk, sigc);
                                }
                                else
                                {
                                    it_sigc_R->second += sigc;
                                }
                            }
                        }
                    }
...
void G0W0::build_sigc_matrix_KS_kgrid(const Atoms &geometry)
{
    ...
    this->build_sigc_matrix_KS(this->mf.get_eigenvectors(), this->pbc.kfrac_list, geometry);
    // if (comm_h.is_root())
    // {
                                            ...
                                            const Atoms &geometry)
{
    ...
    this->build_sigc_matrix_KS_blacs(this->mf.get_eigenvectors(), this->pbc.kfrac_list, geometry, blacs_ctxt_h);
    // if (comm_h.is_root())
    // {
    //     write_self_energy_omega("self_energy_omega.dat", *this);
    // }
