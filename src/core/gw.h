...
bool should_enter_wc_rf_collective(bool output_wc_rf, bool local_frequency_available);
bool direct_compressed_sigc_diagnostic_requested(const char *value);
bool should_contract_sigc_in_compressed_abfs(bool use_shrink_abfs, const char *value);

class G0W0
{
private:
    using SigcRspaceMap = std::map<
        int,
        std::map<int,
                 std::map<int, std::map<double, ap_p_map<std::map<Vector3_Order<int>, Matz>>>>>>;
    ...
    void build_sigc_matrix_KS_blacs(
        const std::map<int, std::map<int, std::map<int, ComplexMatrix>>> &wfc_target,
        const std::vector<Vector3_Order<double>> &kfrac_target,
        const AtomPairBvKRemap<atom_t> &bvk_remap, const BlacsCtxtHandler &blacs_ctxt_h,
        bool use_gpu_replace_scalapack, const std::string &source);
...
public:
    ...
    //! correlation self-energy matrix in the basis of KS states, indices [ispin][ik][freq](local
    //! n_bands, local n_bands)
    std::map<int, std::map<int, std::map<double, Matz>>> sigc_is_ik_f_KS;

    //! correlation self-energy diagonal in the basis of KS states, indices
    //! [ispin][ik][freq](n_bands)
    std::map<int, std::map<int, std::map<double, std::vector<cplxdb>>>> sigc_diag_is_ik_f_KS;

public:
    // Constructors
    G0W0(const MeanField &mf_in, const AtomicBasis &atbasis_wfc_in,
         ...
         bool is_eigvec_k_distributed, bool use_symmetry_context_in = true);
    ...
    void build_spacetime(
        ...
        const BlacsCtxtHandler *blacs_ctxt_h = nullptr, const ArrayDesc *desc_wfc_in = nullptr);

    void build_sigc_matrix_KS_kgrid(const Atoms &geometry = Atoms());
    void build_sigc_matrix_KS_band(
        const std::map<int, std::map<int, std::map<int, ComplexMatrix>>> &wfc_band,
        const std::vector<Vector3_Order<double>> &kfrac_band,
        const AtomPairBvKRemap<atom_t> &bvk_remap, const std::vector<int> *output_iks = nullptr);
    void build_sigc_matrix_KS_kgrid_blacs(const BlacsCtxtHandler &blacs_ctxt_h,
                                          bool use_gpu_replace_scalapack = false);
    void build_sigc_matrix_KS_band_blacs(
        const std::map<int, std::map<int, std::map<int, ComplexMatrix>>> &wfc_band,
        const std::vector<Vector3_Order<double>> &kfrac_band,
        const AtomPairBvKRemap<atom_t> &bvk_remap, const BlacsCtxtHandler &blacs_ctxt_h,
        bool use_gpu_replace_scalapack = false, const std::vector<int> *output_iks = nullptr);
