...
static void handle_KS_file(const string &file_path, MeanField &mf)
{
    ...
    while (infile.peek() != EOF)
    {
        ...
        for (int is = 0; is != nspin; is++)
        {
            set_ao_basis_wfc(is, ik, re + is * n, im + is * n, mf);
        }
        // for abacus
...
size_t handle_Cs_file_binary_by_ids(const string &file_path, double threshold,
    ...
    while ((ptr = readdir(dir)) != NULL)
    {
        ...
        if (fn.find("Cs_data") == 0)
        {
            ...
            for (int id = 0; id < ids_keep_this_file.size(); id++)
            {
                ...
                if (id_global % nprocs == myid)
                    files_Cs_ids_this_proc[fn].push_back(ids_keep_this_file[id]);
            }
